...
if (!function_exists('echo_var')) {
     ...
     * @return void
     */
    function echo_var(mixed $var, $exit = false)
  {
    error_reporting(0);
    $var = check_var($var);
    echo "<pre style='text-align: initial; border-radius: 4px; border: 1px #0c2b45 solid; padding: 4px'>" . PHP_EOL . htmlentities(print_r($var, 1)) . PHP_EOL . "</pre>" . PHP_EOL;
    if ($exit) exit();
...
if (!function_exists('echo_json')) {
  function echo_json($var = '', $exit = false)
  {
    echo_var(json_encode($var, JSON_PRETTY_PRINT), $exit);
  }
}

if (!function_exists('error_to_array')) {
  function error_to_array(Throwable $error): array
  {
    return [
...
if (!function_exists('error_to_string')) {
  function error_to_string(Throwable $error): string
  {
...
if (!function_exists('echo_error')) {
   ...
   * @param bool $exit
   */
  function echo_error($error = '', $exit = false)
  {
    error_reporting(0);
    $var = [
      'class' => get_class($error),
      ...
      'trace' => check_var($error->getTrace())
    ];
...
if (!function_exists('write_log')) {
   ...
   * @param bool $rewrite
   */
  function write_log($var, string $filename = '', $rewrite = false)
  {
    ...
    if (is_string($var)) {
        if (stripos($var, 'SELECT * FROM elemento_configuracionpersonal WHERE asignado = ? AND borrado = 0') !== false
            or stripos($var, 'SELECT * FROM elemento_configuracionpersonal WHERE (asignado = ?) AND (borrado = 0)') !== false)
        {
...
if (!function_exists('array_to_table')) {
    function array_to_table(array|ArrayObject $result_array = [], string $table_attributes = '')
    {
...
if (!function_exists('array_to_table_list')) {
  function array_to_table_list($array, $table_attributes = '')
  {
...
if (!function_exists('get_trace')) {
  function get_trace($num_lines = 0, $offset = 2)
  {
    $trace = [];
...
if (!function_exists('get_trace_string')) {
  function get_trace_string($num_lines = 0, $offset = 2)
  {
    $trace = "";
...
if (!function_exists('strtodate')) {
  ...
  function strtodate($string = '')
  {
    ...
    array_walk($arry, function ($val, $key) {
      if ($key >= 1) $arry[$key] = str_pad($val, 2, '0', STR_PAD_LEFT);
    });
...
if (!function_exists('getDirectoryTree')) {
   ...
   * @return array
   */
  function getDirectoryTree(string $directory)
  {
    ...
    if (file_exists($directory)) {
      ...
      foreach ($list as $i => $item) {
        ...
        $type = filetype($path);
        $tree[$i] = array_merge(['name' => $item, 'type' => $type, 'date' => date(DateTime::ATOM, filemtime($path))], pathinfo($path));
        switch ($type) {
          case 'dir':
...
if (!function_exists('flattenDirectoryTree')) {
  /**
   * @param string $directory
   * @param array $tree
   * @return array
   */
  function flattenDirectoryTree(string $directory, array $tree, $relative = false)
  {
    $flattened = [];
    $directory = rtrim($directory, '/');
...
if (!function_exists('reemplazarSecuencial')) {
    function reemplazarSecuencial($cadena, $caracter, array $valores, $envoltura = '') {
        $resultado = '';
        $contador = 0;
